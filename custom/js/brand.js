...
$(document).ready(function() {
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
		...
		if(rif == "") {
			$("#rif").after('<p class="text-danger">Este campo es obligatorio</p>');
			$('#rif').closest('.form-group').addClass('has-error');
		} else {
			$("#rif").find('.text-danger').remove();
			$("#rif").closest('.form-group').addClass('has-success');	  	
		}
	...
	if(brandId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				// setting the rif value
				$('#editRif').val(response.rif);
				// setting the brand status value
				$('#editBrandStatus').val(response.brand_status); // Usar brand_status
				// brand id 
