...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			var locationDestination = $("#locationDestination").val(); // Capturar el valor de locationDestination
			var observation = $("#observation").val(); // Capturar el valor de observation

			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> La fecha de la orden es obligatoria </p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> El nombre del analista es obligatorio </p>');
			...
			if(locationDestination == "") {
				$("#locationDestination").after('<p class="text-danger"> La localidad de destino es obligatoria </p>');
				$('#locationDestination').closest('.form-group').addClass('has-error');
			} else {
				$('#locationDestination').closest('.form-group').addClass('has-success');
			} // /else

			// La observación es opcional, no requiere validación estricta aquí a menos que se especifique
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && locationDestination && validateProduct && validateQuantity) { // Añadido locationDestination a la validación
					// create order button
					...
					$.ajax({
						...
						success:function(response) {
							console.log("Respuesta del servidor (createOrder):", response);
							// reset button
	            	...
	            	'<a href="orders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Agregar nueva salida </a>'+
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			...
			'columns': [
				...
				{ "data": 1 }, // Fecha
				{ "data": 2 }, // Analista
				{ "data": 3 }, // Total de productos
				{ "data": 4, "render": function ( data, type, row ) { return data; } }  // Opciones (botón de acción)
			]
		});		
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> El nombre del cliente es obligatorio </p>');
				$('#clientName').closest('.form-group').addClass('has-error');
			} else {
				$('#clientName').closest('.form-group').addClass('has-success');
			} // /else
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger"> El contacto del cliente es obligatorio </p>');
			...
			if(paid == "") {
				$("#paid").after('<p class="text-danger"> El monto pagado es obligatorio </p>');
			...
			if(discount == "") {
				$("#discount").after('<p class="text-danger"> El descuento es obligatorio </p>');
			...
			if(paymentType == "") {
				$("#paymentType").after('<p class="text-danger"> El tipo de pago es obligatorio </p>');
			...
			if(paymentStatus == "") {
				$("#paymentStatus").after('<p class="text-danger"> El estado de pago es obligatorio </p>');
			...
			for (var i = 0; i < productName.length; i++) {       			
				var productNameId = productName[i].id;	    	
		    if(productName[i].value == "") {
		    	$("#"+ productNameId +").after('<p class="text-danger">El nombre del producto es obligatorio</p>');
		    	$("#"+ productNameId +").closest('.form-group').addClass('has-error');	
		    	validateProduct = false;
		    } else { 			
		    	$("#"+ productNameId +").closest('.form-group').addClass('has-success');	
		...
		for (var i = 0; i < quantity.length; i++) {
			var quantityId = quantity[i].id;		
			if(quantity[i].value == "") {
				$("#"+ quantityId +").after('<p class="text-danger">La cantidad es obligatoria</p>');
				$("#"+ quantityId +").closest('.form-group').addClass('has-error');	
				validateQuantity = false;
			} else {
				$("#"+ quantityId +").closest('.form-group').addClass('has-success');	
		...
		if(orderDate && clientName && clientContact && locationDestination && paid && discount && paymentType && paymentStatus) { // Añadido locationDestination a la validación
			if(validateProduct == true && validateQuantity == true) {
				...
				$.ajax({
					...
					dataType : 'json',
					success:function(response) {
						console.log("Respuesta del servidor (editOrder):", response);
...
function subAmount() {
	...
	for(x = 0; x < tableProductLength; x++) {
		...
		totalProducts = Number(totalProducts) + Number($("#quantity"+count).val());
	} // /for
